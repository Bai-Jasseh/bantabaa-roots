...
export const Route = createFileRoute("/opportunities")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Jobs, contracts, grants, and mentorship — built for West African developers. No noise, just opportunity." },
...
function OpportunitiesPage() {
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = { All: SAMPLE_OPPORTUNITIES.length };
    SAMPLE_OPPORTUNITIES.forEach((o) => { c[o.type] = (c[o.type] ?? 0) + 1; });
    return c;
  }, []);

  const filtered = useMemo(() => SAMPLE_OPPORTUNITIES
    ...
    <div>
      ...
      <div className="mx-auto max-w-6xl px-4 pt-12 md:px-6 md:pt-16">
        ...
        >
          ...
          <p className="mt-3 text-lg text-muted-foreground">
            Jobs, contracts, grants, and mentorship built for West African developers. No noise. Just opportunity.
          </p>
        ...
        <div className="mt-10 grid grid-cols-3 gap-4 rounded-2xl border border-border bg-[var(--cream)] p-6 dark:bg-card">
          <Stat value={SAMPLE_OPPORTUNITIES.length * 8} label="Opportunities Active" />
          <Stat value={32} label="Companies Hiring" />
          <Stat value={184} label="Developers Placed" />
        </div>
      </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-3 md:px-6">
              ...
              {TABS.map((t) => (
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition-all duration-200 ${
                    tab === t.key
                      ? "bg-[var(--baobab)] text-[var(--baobab-foreground)] shadow-sm"
                      : "text-muted-foreground hover:bg-secondary hover:text-foreground"
                  }`}
                >
                  {t.label}
                  <span className={`rounded-full px-2 py-0.5 text-[11px] ${tab === t.key ? "bg-white/15 text-white" : "bg-[var(--kola)]/15 text-[var(--kola)]"}`}>
                    {counts[t.key] ?? 0}
                  </span>
                </button>
              ))}
          ...
          {activeFilters.length > 0 && (
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              ...
              <button onClick={() => { setLocation("All"); setExperience("All"); setComp("All"); }} className="text-xs font-medium text-muted-foreground hover:text-[var(--kola)]">
                Clear all
              </button>
            </div>
          )}
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 md:px-6">
        <AnimatePresence mode="wait">
          <motion.div
            key={`${tab}-${location}-${experience}-${comp}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            {featured && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }}>
                <OpportunityCard op={featured} featured />
              </motion.div>
            )}
        ...
        {filtered.length === 0 && (
          <div className="rounded-3xl border border-dashed border-border bg-card p-12 text-center">
            ...
            <p className="mt-1 text-muted-foreground">Try adjusting or clearing them.</p>
            <Button onClick={() => { setLocation("All"); setExperience("All"); setComp("All"); }} variant="outline" className="mt-4">
              Clear Filters
            </Button>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Link to="/for-companies">
            <Button className="bg-[var(--kola)] text-[var(--kola-foreground)] hover:bg-[var(--kola)]/90">
              Post now <ArrowRight className="size-4" />
            </Button>
          </Link>
        </div>
      </div>
